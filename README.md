...
`londonSweptBothAsia`, `nyPremarketClearedBothLondon`

Cada evento fica "vivo" 2 horas no RDM Clearance (24h para os dois de sweep de sessão,
já que só interessam para o dia mais recente).

### Sweeps de sessão sem precisar da Tradovate

`pine/session-sweeps.pine` neste repo é um indicador Pine Script pronto a colar num
gráfico de MNQ/MES no TradingView. Calcula sozinho os highs/lows de Asia, London e NY
premarket e dispara os dois alertas (`londonSweptBothAsia`, `nyPremarketClearedBothLondon`)
com o JSON já formatado — troca só `O_TEU_WEBHOOK_SECRET` pelo valor real. Isto evita
teres de pagar a subscrição de API da Tradovate.
